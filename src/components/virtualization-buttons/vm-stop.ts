...
export const run: Buttons["run"] = async ({ interaction, client, optionalParams }) => {
    ...
    try {
        ...
        const res = await vmManager.executeVMAction(panelId, { type: "stop", vmId: id }, interaction.user.id);

        if (!res.success) {
            await interaction.followUp({
                content: `❌ Falló detener la VM: ${res.error}`,
                flags: MessageFlags.Ephemeral
            });
        }
    } catch (error) {
        client.logger.error({ error }, "Error in VM stop handler");
        await interaction.followUp({
